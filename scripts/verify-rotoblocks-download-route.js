// Guards the Roto Blocks smart download page, the same way
// verify-download-route.js guards the Jelly Chain Rush one: /rotoblocks/download
// is the single link that gets shared publicly, so a wrong storefront code, a
// JS-only store button, or an auto-redirect that fires before the fallback UI
// can render is a production incident, not a cosmetic bug.
//
// Four halves:
//   1. Device detection — getStoreDestination/getAutoRedirectDestination and
//      the pre-hydration inline script must agree for every user agent we care
//      about, including in-app browsers (Instagram, WhatsApp, TikTok,
...
//      anchor hrefs, so the page still works with no JavaScript and no
//      successful auto-redirect at all.
//   3. Storefront — no Roto Blocks App Store link anywhere in the source or
//      the export may point at a storefront other than the published one. A
//      two-letter country code silently breaks every Apple device while
//      leaving Android perfectly healthy.
//   4. Route shape — the hyphenated /roto-blocks/download must never be the
//      link that ships; /roto-blocks stays the marketing page only.
...
// under plain Node on Cloudflare with no TypeScript loader.
const {
  APP_STORE_URL,
  GOOGLE_PLAY_URL,
  REDIRECT_FLAG,
  ...
  isInAppBrowser,
} = require("../src/lib/rotoBlocksDeviceRedirect.cjs");

const FALLBACK = null;

// Written out literally so the check cannot drift along with the constant it
// is guarding. Countryless on purpose.
const CANONICAL_APP_STORE_URL = "https://apps.apple.com/app/id6797314822";
...
// Any apps.apple.com link carrying the Roto Blocks app id, whatever storefront
// it claims.
const ROTO_APP_STORE_LINK = /https:\/\/apps\.apple\.com\/\S*?id6797314822/g;
...
// the script must *schedule* a delayed navigation, never call location.href
// synchronously.
function runInlineScript(userAgent, maxTouchPoints) {
  const scheduled = [];
  const windowStub = {
    navigator: { userAgent, maxTouchPoints, vendor: "" },
    location: { href: "https://lumisoftstudios.com/rotoblocks/download/" },
    setTimeout: (fn, delay) => {
...
// in two places. Read as text rather than required as a module: products.ts is
// TypeScript and this script must run under plain Node with no TS loader.
const productsSourcePath = path.resolve("src/lib/products.ts");
const productsSource = fs.readFileSync(productsSourcePath, "utf8");
const rotoBlocksBlockMatch = productsSource.match(
  /rotoBlocks:\s*\{([\s\S]*?)\n\s*\},/,
);

if (!rotoBlocksBlockMatch) {
  failures.push(
    `${productsSourcePath}: could not locate the rotoBlocks STORE_URLS entry`,
  );
} else {
  const block = rotoBlocksBlockMatch[1];
  ...
  if (!appStoreMatch || appStoreMatch[1] !== CANONICAL_APP_STORE_URL) {
    failures.push(
      `src/lib/products.ts: rotoBlocks.appStore is ${appStoreMatch ? appStoreMatch[1] : "missing"}`,
    );
  }
  if (!googlePlayMatch || googlePlayMatch[1] !== CANONICAL_GOOGLE_PLAY_URL) {
    failures.push(
      `src/lib/products.ts: rotoBlocks.googlePlay is ${googlePlayMatch ? googlePlayMatch[1] : "missing"}`,
    );
  }
}

// Roto Blocks must be advertised as live: a stale "coming-soon" status would
// keep the card badge wrong on both the English and Turkish home pages.
if (!/slug:\s*"roto-blocks",[\s\S]*?status:\s*"live",/.test(productsSource)) {
  failures.push(
    'src/lib/products.ts: the roto-blocks product is not marked status: "live"',
  );
}
...
// Sweep every authored source file and every exported artifact, so a new page
// that hardcodes its own Roto Blocks link is caught too.
...
for (const root of scanRoots) {
  for (const file of walk(root)) {
    ...
    // The hyphenated route was never built; it must not appear as a link.
    if (contents.includes("/roto-blocks/download")) {
      failures.push(
        `${displayName}: references the wrong route /roto-blocks/download — the shared link is /rotoblocks/download`,
      );
    }

    if (!contents.includes("id6797314822")) continue;

    scannedFiles += 1;

    for (const link of new Set(contents.match(ROTO_APP_STORE_LINK) ?? [])) {
      // Trim the trailing punctuation the greedy scan can pick up in JS bundles.
      const cleaned = link.replace(/[\\"'`,;)\]}]+$/, "");
      if (cleaned !== CANONICAL_APP_STORE_URL) {
        failures.push(
          `${displayName}: non-canonical Roto Blocks App Store link ${cleaned}`,
        );
      }
    }
  }
...
const pagePath = path.resolve("out", "rotoblocks", "download", "index.html");
...
if (!fs.existsSync(pagePath)) {
  ...
  if (!html.includes('id="rb-store-redirect"')) {
    failures.push(
  ...
  if (
    /pointer-events-none[^"]*"[^>]*href="https:\/\/(apps\.apple|play\.google)/.test(
      html,
    )
  ) {
  ...
  if (
    /href="https:\/\/(apps\.apple|play\.google)[^"]*"[^>]*onclick=/i.test(html)
  ) {
    failures.push(
      "fallback markup: a store anchor relies on a JS onclick handler",
    );
  }
}

if (failures.length > 0) {
  console.error(`Roto Blocks download route check failed (${failures.length}):`);
  for (const failure of failures) console.error(`- ${failure}`);
  process.exit(1);
}

console.log(
  `✓ Verified Roto Blocks download route: ${cases.length} device-detection cases (store + in-app + auto-redirect + inline script), fallback anchor markup, ${REDIRECT_DELAY_MS}ms redirect delay, the hyphen-free /rotoblocks/download route, and ${scannedFiles} files carrying the App Store link — all on the canonical countryless storefront.`,
);
